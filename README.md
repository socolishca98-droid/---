...
cd loginex
# ветка main — в ней лежит вся программа целиком, переключаться никуда не нужно
...
* `main` — **вся программа целиком**: авторизация и безопасность, схема с
  организациями, документы, клиенты и оплаты, отчёты, OCR, кеш и интерфейс.
  Для локального запуска достаточно её, переключаться никуда не нужно.
* Рабочие ветки вида `arena/*` — ветки, в которых велась разработка; они
  вливаются в `main` через Pull Request. Если вы клонировали репозиторий до
  слияния очередного PR, просто выполните `git pull` в `main`.

Проверить, что у вас на диске полная версия, можно скриптом:
