 ...
 *
 * Действия:
 *   approve       — одобрить заявку на присоединение (pending → active)
 *   reject        — отклонить заявку (запись удаляется, использование инвайт-кода
 *                   возвращается через releaseInvite)
 ...
 *
 * Кто что может (внутри своей организации):
 *   approve, reject — ЛОГИСТ И АДМИНИСТРАТОР. Решение пользователя от 2026-09-24:
 *                     «Все заявки может одобрять логист и админ». Роль будущего
 *                     сотрудника задаёт администратор при создании инвайт-кода —
 *                     одобрение лишь активирует учётную запись.
...
/** Заявки на присоединение: обрабатывает любой сотрудник организации (логист или
 *  администратор). Остальные действия — только администратор. */
const APPLICATION_ACTIONS: Action[] = ["approve", "reject"]
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  // Заявки (approve/reject) одобряет и отклоняет любой сотрудник организации —
  // логист или администратор. Закрытие/восстановление доступа, смена роли,
  // сброс пароля и снятие блокировки — только администратор организации.
  if (!org.isAdmin && !APPLICATION_ACTIONS.includes(action)) {
    return NextResponse.json(
      {
        success: false,
