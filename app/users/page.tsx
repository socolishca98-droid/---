 ...
 *
 * Здесь логист:
 *  • одобряет заявки на регистрацию (pending → active),
 *  • закрывает доступ уволенному сотруднику (active → suspended, с указанием причины;
 *    все его сессии немедленно отзываются, запись в базе остаётся),
 *  • восстанавливает доступ (suspended → active),
 *  • сбрасывает пароль (пользователь обязан сменить его при следующем входе),
 *  • меняет роль (только администратор).
 */
