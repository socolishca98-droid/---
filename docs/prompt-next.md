...
### Открытые развилки задачи 1 (жду решения, сам не решаю)
- **(a)** «Отклонить» на `/users` делает `suspend` (запись остаётся, использование кода
  сгорает), а на `/organization` — `reject` (заявка удаляется, использование кода возвращается).
  Унифицировать на `reject`, оставить как есть или дать обе кнопки?
- **(b)** Назначать роль `admin`/`logist` прямо в момент одобрения (сейчас роль приходит из
  инвайт-кода и меняется отдельным действием `setRole`, которое доступно только админу)?
- **(d)** Два экрана со списком сотрудников и заявками (`/users` и `/organization`) —
  разделить ответственность, объединить в один или оставить оба?
